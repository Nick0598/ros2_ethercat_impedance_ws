  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  uint32_t previous_working_counter = 0;
  bool has_previous_working_counter = false;
  bool domain_stable = false;
  RCLCPP_INFO(
    rclcpp::get_logger("EthercatDriver"),
    "Waiting for EtherCAT domain to stay COMPLETE for %u cycles before CiA402 enable "
    "(timeout: 60 s).",
    required_stable_cycles);

  for (uint32_t startup_cycle = 0; startup_cycle < max_startup_cycles; ++startup_cycle) {
    ...
    previous_working_counter = working_counter;
    has_previous_working_counter = true;
  ...
  RCLCPP_INFO(
    rclcpp::get_logger("EthercatDriver"),
    "EtherCAT domain stable with WC %u. Releasing CiA402 auto state transitions.",
    previous_working_counter);
  for (auto & module : ec_modules_) {
