...
private:
    ...
    const control_msgs::msg::JointTrajectoryControllerState::SharedPtr msg)
  {
    ...
    if (msg->joint_names.size() != msg->feedback.positions.size()) {
      RCLCPP_WARN_THROTTLE(
        get_logger(), *get_clock(), 2000,
        "Controller state joint_names size (%zu) does not match feedback.positions size (%zu); "
        "ignoring message.",
        msg->joint_names.size(), msg->feedback.positions.size());
      return;
    }
    ...
    for (std::size_t i = 0; i < joint_names_.size(); ++i) {
      ...
      q_current[i] = msg->feedback.positions[it->second];
    }
